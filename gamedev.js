document.addEventListener('DOMContentLoaded', () => {
    ...
    projects.forEach(project => {
        project.addEventListener('click', (e) => {e.preventDefault();const href = project.getAttribute('data-href');if (href) {pageTransition(href);}});
        project.addEventListener('mouseenter', () => {project.style.transform = 'translateY(-10px)';project.style.transition = 'all 0.3s ease';});
    ...
    document.querySelectorAll('a').forEach((cvLink) => {cvLink.addEventListener('click', (e) => {e.preventDefault();const href = cvLink.getAttribute('href');pageTransition(href);});});      
    ...
    window.addEventListener('popstate', () => {overlay.style.transform = 'translateX(100%)';overlay.style.transition = 'none';});
    window.addEventListener('error', () => {overlay.style.transform = 'translateX(100%)';overlay.style.transition = 'none';});
});